...
-- The existing vessel_daily_cost_summary deliberately contains the numeric
-- total only. Currency is taken from the vessel financial baseline or the
-- project cost line items here.
CREATE OR REPLACE VIEW public.vessel_project_cost_summary AS
SELECT
  v.id AS vessel_id,
  v.org_id,
  COALESCE(dc.total_daily_vessel_operating_cost, 0)::numeric(14,2) AS daily_operating_cost,
  COALESCE(SUM(pc.amount), 0)::numeric(14,2) AS project_specific_costs,
  COUNT(pc.id)::integer AS project_cost_line_count,
  COALESCE(MAX(f.currency_code), MAX(pc.currency_code), 'USD') AS currency_code
...
GROUP BY v.id, v.org_id, dc.total_daily_vessel_operating_cost;
...
AS $$
  SELECT
    ...
    (COALESCE(dc.total_daily_vessel_operating_cost, 0) * GREATEST(COALESCE(p_project_days, 0), 0))::numeric(14,2),
    COALESCE((
    ...
    (
      COALESCE(dc.total_daily_vessel_operating_cost, 0) * GREATEST(COALESCE(p_project_days, 0), 0)
      + COALESCE((
          SELECT SUM(pc.amount)
          FROM public.vessel_project_costs pc
          WHERE pc.vessel_id = v.id
...
COMMENT ON TABLE public.vessel_project_costs IS
  'Project-specific vessel costs. Combined with every daily vessel operating cost in vessel_daily_cost_summary to calculate total project cost.';
