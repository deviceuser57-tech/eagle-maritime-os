...
import VesselManagementV2 from '@/components/VesselManagementV2';
import VesselProjectCostTable from '@/components/VesselProjectCostTable';
...
const IndexContent = () => {
  ...
  useEffect(() => {
    if (user?.id && orgId) seedSetupData(orgId, user.id);
  }, [user?.id, orgId]);
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'setup': return <><SetupPage /><VesselSetupMasters /></>;
      case 'vessel-management': return <VesselManagementV2 />;
      case 'vessel-project-costs': return <VesselProjectCostTable />;
        ...
        return (
          <div className="space-y-6">
            ...
            <p className="text-muted-foreground font-medium">
              This section is currently being integrated into the maritime node.
            </p>
          </div>
        );
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-primary/20 border-t-primary rounded-full animate-spin mx-auto mb-6 shadow-glow" />
          <p className="text-foreground text-sm font-black uppercase tracking-[0.3em] animate-pulse">
            Syncing Vessel Core...
          </p>
        </div>
      </div>
  ...
  if (showFrontPage) return <FrontPage onEnterDashboard={() => setShowFrontPage(false)} />;
  if (!user) return <AuthPage onAuthSuccess={() => console.log('Authentication secure. Access granted.')} />;
      ...
      <main className="flex-1 overflow-y-auto relative scrollbar-hide bg-background">
        ...
        <div className="relative z-10 p-6 sm:p-8 lg:p-10">
          <div className="max-w-[1600px] mx-auto">
            ...
            <UniversalSearch onSearch={(q) => console.log('Search:', q)} />
...
const Index = () => <AuthProvider><IndexContent /></AuthProvider>;
